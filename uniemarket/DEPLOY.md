# Deploy Uniemarket lên Cloudflare Pages (miền riêng + chống DDoS mạnh nhất)
...
2. Push code lên (thư mục gốc chứa `uniemarket/`).
3. Đảm bảo **KHÔNG** commit `uniemarket/.env.local` (đã nằm trong `.gitignore` — khóa nằm trong biến môi trường của Pages, không nằm trong repo).
...
2 file đã có sẵn trong `uniemarket/public/` (Vite tự copy vào `dist/`):
