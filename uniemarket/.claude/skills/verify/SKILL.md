---
name: verify
description: Build, launch, and drive the Uniemarket demo (Vite React SPA) end-to-end with Playwright
---
...
4. `/cart` → promo `UNIE10` gives 10% off (wrong code → error toast).
5. `/checkout` → fill email + Roblox username → "Đặt hàng" → `/order-success`
   with order id `UM-YYYY-NNNN`; cart clears after success.
6. Login demo: `demo@uniemarket.gg` / `demo1234` (or "Điền nhanh" button) → `/dashboard`.
...
- State lives in localStorage keys `uniemarket-*`; clear them (or footer "Reset demo",
